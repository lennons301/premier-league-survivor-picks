-- Fix elimination logic to include draws after the first gameweek
...
BEGIN
  ...
  IF NEW.finished = true AND (OLD.finished IS NULL OR OLD.finished = false) THEN
    ...
    LOOP
      ...
      IF NOT is_win THEN
        -- Only allow rebuy if this non-win occurred in the starting gameweek of the game
        IF pick_record.gameweek = pick_record.starting_gameweek THEN
          ...
          NULL;
        ELSE
          -- Immediate elimination for non-wins (losses OR draws) after the first gameweek
